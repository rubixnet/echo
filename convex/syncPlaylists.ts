...
export const CATEGORY_PLAYLISTS = [
  ...
  {
    id: "metalcore-nu-metal",
    name: "Metalcore Nu-Metal",
  ...
  {
    id: "boom-bap",
    name: "Boom Bap Old School Hip-Hop",
  ...
  {
    id: "traditional-swing-jazz",
    name: "Traditional Swing Jazz",
  ...
  {
    id: "delta-chicago-blues",
    name: "Delta Chicago Blues",
  ...
  {
    id: "modern-pop-country",
    name: "Modern Pop Country",
  ...
  },
  {
    ...
    name: "Baroque Classical Era",
    playlistId: "PLcGkkXtask_clYSk4gXUAjfpquAo_lE9W",
    type: "genre",
    syncFrequency: "weekly",
  },
  {
    id: "romantic-classical",
    name: "Romantic Era Classical",
  ...
  {
    id: "modern-classical",
    name: "Modern Classical Minimalist",
  ...
  {
    id: "salsa-bachata",
    name: "Salsa Bachata Merengue",
    playlistId: "PLWQLazBs82Gg6-VUJC6QUhY0crkjaxWWZ",
    type: "genre",
    syncFrequency: "weekly",
  },
  {
    id: "reggaeton-latin-trap",
  ...
  {
    id: "noise-experimental",
    name: "Noise Experimental",
...
export const syncPlaylistsByFrequency = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (!apiKey) {
      console.error(
        "[CRITICAL] YOUTUBE_API_KEY missing in Convex environment.",
      );
      return;
    }
    ...
    const filteredPlaylists = CATEGORY_PLAYLISTS.filter(
      (item) =>
        targetFrequency === "all" || item.syncFrequency === targetFrequency,
    );

    console.log(
      `[SYNC RUN] Processing ${filteredPlaylists.length} categories (Mode: ${targetFrequency})...`,
    );
    ...
    for (const item of filteredPlaylists) {
      try {
        ...
        if (
          ...
          console.warn(
            `[SYNC SKIP] No items returned for "${item.name}". Message: ${playlistData.error?.message || "Empty payload"}`,
          );
          continue;
        }
        ...
        console.log(
          `[SYNC SUCCESS] Saved ${tracks.length} tracks for "${item.name}"`,
        );
      } catch (err) {
