...
export default function SidebarLayout({ children }: { children: React.ReactNode }) {
    ...
    const historySongs = useQuery(api.history.getUserHistory, user?._id ? { userId: user._id } : "skip");
    ...
    return (
        <div className="flex bg-background text-foreground overflow-hidden">
            <aside className="hidden lg:flex relative  w-72 md:w-80 shrink-0 h-full">
                <div className="absolute right-0 top-10 bottom-0 w-px bg-foreground/10 pointer-events-none" />
                <div className="flex-1 flex flex-col w-full px-4 pb-2 pt-10 overflow-y-auto liquid-scroll">
                    {historySongs.length > 0 && (
                        <>
                            <Link
                                href="/dashboard/library/history"
                                className={cn(
                                    "flex items-center gap-3 m-1 p-1 rounded-lg  transition-colors group",
                            ...
                            >
                                ...
                                <div className="flex flex-col min-w-0">
                                    <span className={cn(
                                        ...
                                        pathname === "/dashboard/library/history" ? "text-foreground" : "text-foreground/80 group-hover:text-foreground"
                                    )}>
                                        Recently Played
                                    </span>
                        ...
                        <>
                            ...
                            >
                                ...
                                <div className="flex flex-col min-w-0">
                                    <span className={cn(
                                        "text-sm font-bold truncate transition-colors",
                                        pathname === "/dashboard/library/liked" ? "text-foreground" : "text-foreground/80 group-hover:text-foreground"
                                    )}>
                                        Liked Songs
                                    </span>
                                    <span className="text-xs font-medium text-foreground/50 truncate">
            ...
            <main className="flex-1 min-w-0 h-full overflow-y-auto liquid-scroll">
                {children}
            </main>
        </div>
    );
...
function SidebarPlaylistItem({ playlist, isActive, isLast }: { playlist: any, isActive: boolean, isLast: boolean }) {
    const tracks = useQuery(api.playlists.getPlaylistTracks, { playlistId: playlist._id });

    const trackCount = tracks?.length || 0;
    const coverUrl = playlist.coverUrl || (tracks && tracks.length > 0 ? tracks[0]?.coverUrl : null);
        ...
        <React.Fragment>
            ...
            >
                <div className="w-12 h-12 shrink-0 rounded-md bg-foreground/5 border border-foreground/10 flex items-center justify-center overflow-hidden shadow-sm">
                    {coverUrl ? (
                        <img src={coverUrl} className="w-full h-full object-cover" alt={playlist.name} />
                    ) : (
