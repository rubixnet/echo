...
import {
  ...
  SkipForward, SkipBack, Volume2, LogOut, PanelLeftClose, PanelLeftOpen,
...
export default function ClientLayout({
  ...
  const { progressRef, isPlaying, currentTimeStr, duration, togglePlay, seek } = useAudioEngine();
  ...
  const handleCreateRoom = () => {
    ...
    router.push(`/dashboard/room/${mockRoomId}`);
  };
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-neutral-50/50 font-sans overflow-hidden selection:bg-emerald-200 selection:text-emerald-900">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside
          className={cn(
            "bg-neutral-50/80 border-r border-neutral-200/60 flex flex-col justify-between hidden md:flex transition-all duration-300 ease-in-out shrink-0 z-30",
          ...
          <div className="flex flex-col h-full">
            <div className={cn("h-[90px] flex items-center px-4 border-b border-transparent", isCollapsed ? "justify-center" : "justify-between")}>
              {!isCollapsed && (
                <Link href="/dashboard" className="flex items-center gap-2 px-2">
                  <div className="w-8 h-8 bg-neutral-900 rounded-lg flex items-center justify-center text-white font-black text-sm">
                    E♪
                  </div>
            ...
            <div className="p-4 flex-1 overflow-y-auto space-y-6">
              <nav className="space-y-1.5">
                {navItems.map((item) => (
                  <Link
                    ...
                    className={cn(
                      "flex items-center rounded-xl text-sm font-bold transition-all h-10",
              ...
              <div>
                ...
                <nav className="space-y-1.5">
                  <Link href="/dashboard/liked" title={isCollapsed ? "Liked Songs" : ""} className={cn("flex items-center rounded-xl text-sm font-bold text-neutral-500 hover:text-neutral-900 hover:bg-white/60 transition-all h-10", isCollapsed ? "justify-center" : "gap-3 px-3", pathname === "/dashboard/liked" && "bg-white text-neutral-900 shadow-sm border border-neutral-200/50")}>
                    <Heart size={18} strokeWidth={pathname === "/dashboard/liked" ? 2.5 : 2} className="shrink-0" />
                    {!isCollapsed && <span>Liked Songs</span>}
                  </Link>
                  <Link href="/dashboard/rooms" title={isCollapsed ? "Live Rooms" : ""} className={cn("flex items-center rounded-xl text-sm font-bold text-neutral-500 hover:text-neutral-900 hover:bg-white/60 transition-all h-10", isCollapsed ? "justify-center" : "gap-3 px-3", pathname === "/dashboard/rooms" && "bg-white text-neutral-900 shadow-sm border border-neutral-200/50")}>
              ...
              <div className="space-y-2">
                {!isCollapsed && (
                  <p className="px-3 text-[10px] font-bold uppercase tracking-wider text-neutral-400 mb-2">Live Sync Jam</p>
                )}
                ...
                {activeRoom ? (
                  <div className={cn("bg-neutral-900 text-white rounded-2xl p-4 relative overflow-hidden shadow-md", isCollapsed && "p-2 flex flex-col items-center")}>
                    ...
                    {!isCollapsed ? (
                      <>
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            ...
                            className="h-7 text-[11px] font-bold rounded-lg flex-1 bg-white/10 hover:bg-white/20 text-white border-0"
                          >
                          ...
                          <Button
                            ...
                            className="h-7 w-7 rounded-lg bg-rose-500/20 text-rose-300 hover:bg-rose-500 hover:text-white border-0"
                            title="Close Session"
                          >
                            <X size={12} />
                          </Button>
                      ...
                      <Button
                        ...
                        size="icon"
                        onClick={handleCloseRoom}
                        className="h-8 w-8 rounded-xl bg-rose-500/20 text-rose-400 hover:bg-rose-500 hover:text-white"
                        title="Close active room"
                      >
                  ...
                  <div className="bg-white border border-neutral-200 rounded-2xl p-3 space-y-2.5 shadow-sm">
                    <input
                      ...
                      className="w-full text-xs font-medium px-3 py-1.5 bg-neutral-50 border border-neutral-200 rounded-lg outline-none focus:border-neutral-400 transition-colors"
                    />
                    <div className="flex items-center gap-1.5">
                      <Button size="sm" onClick={handleCreateRoom} className="h-7 text-[11px] font-bold flex-1 bg-neutral-900 text-white hover:bg-neutral-800 rounded-lg">
                        Launch
                      </Button>
                      <Button size="sm" variant="ghost" onClick={() => setIsCreating(false)} className="h-7 w-7 p-0 rounded-lg text-neutral-400 hover:text-neutral-900">
                        <X size={14} />
                      </Button>
                    </div>
                  </div>
                  ...
                  <Button
                    ...
                    variant="outline"
                    className={cn("w-full h-10 border-dashed border-neutral-300 text-neutral-500 hover:text-neutral-900 hover:bg-white rounded-xl text-xs font-bold shadow-none", isCollapsed ? "p-0 justify-center" : "gap-2 px-3 justify-start")}
                  >
            ...
            <div className="p-4 border-t border-neutral-200/60 bg-neutral-50/50">
              <div className={cn("flex items-center", isCollapsed ? "flex-col gap-4 justify-center" : "justify-between")}>
                <div className={cn("flex items-center", isCollapsed ? "justify-center" : "gap-3")}>
                  ...
                  {!isCollapsed && (
                    <div className="flex flex-col min-w-0 text-sm font-bold text-neutral-900 truncate">
                      {user?.email.split('@')[0] || "Unknown User"}
                    </div>
                  )}
                </div>
                <Link href="/api/auth/logout">
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-neutral-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-colors" title="Sign Out">
                    <LogOut size={16} />
                  </Button>
                </Link>
              </div>
      ...
      <div className="h-[90px] shrink-0 w-full bg-white/80 backdrop-blur-3xl border-t border-neutral-200/60 flex items-center justify-between px-4 md:px-8 z-50">
        ...
        <div className="flex items-center gap-3 md:gap-4 w-1/4 md:w-1/3">
          ...
          <div className="hidden sm:block min-w-0">
            <h4 className="text-sm font-bold text-neutral-900 truncate">We fell in love in october</h4>
            <p className="text-xs font-medium text-neutral-500 truncate">girl in red</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 md:gap-4 w-1/4 md:w-1/3">
          {activeRoom && (
            <div className="hidden lg:flex items-center gap-1.5 mr-4 px-2.5 py-1 bg-neutral-100 rounded-lg text-neutral-600 text-[11px] font-bold border border-neutral-200/50">
              <Shield size={12} className="text-emerald-500" /> Host Engine Active
            </div>
          )}
          <button className="text-neutral-400 hover:text-neutral-900 hidden sm:block"><Volume2 size={18} /></button>
          <div className="w-16 md:w-24 h-1.5 bg-neutral-200/80 rounded-full overflow-hidden cursor-pointer hidden sm:block hover:h-2 transition-all">
