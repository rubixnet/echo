...
export default function ClientLayout({ children, user }: { children: React.ReactNode; user: any; }) {
  ...
  return (
    <UserProvider user={user}>
      <div className="flex flex-col h-screen w-full bg-neutral-50/50 font-sans overflow-hidden selection:bg-emerald-200 selection:text-emerald-900 text-neutral-900">
        <div className="flex flex-1 overflow-hidden">
          
          {/* SIDEBAR - CLEAN PREMIUM LIGHT THEME */}
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="p-4 flex-1 overflow-y-auto space-y-6">
                ...
                <div>
                  ...
                  <nav className="space-y-1.5">
                    <Link href="/dashboard/rooms" className={cn("flex items-center rounded-xl text-sm font-bold transition-all h-10", isCollapsed ? "justify-center px-0" : "gap-3 px-3", pathname === "/dashboard/rooms" ? "bg-white text-neutral-900 shadow-sm border border-neutral-200/50" : "text-neutral-500 hover:text-neutral-900 hover:bg-white/60")}>
                      ...
                      {!isCollapsed ? (
                        <>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Button variant="destructive" size="icon" onClick={handleCloseRoom} className="h-7 w-7 rounded-lg bg-rose-500/20 text-rose-300 hover:bg-rose-50 hover:text-white border-0">
                              <X size={12} />
                            </Button>
                          </div>
                        </>
              ...
              <div className="p-4 border-t border-neutral-200/60 bg-neutral-50/50">
                <div className={cn("flex items-center", isCollapsed ? "flex-col gap-4 justify-center" : "justify-between")}>
                  ...
                  <Link href="/api/auth/logout" className={cn(buttonVariants({ variant: "ghost", size: "icon" }), "h-8 w-8 text-neutral-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-colors")}>
                    <LogOut size={16} />
                  </Link>
                </div>
              </div>
          ...
          <main className="flex-1 overflow-y-auto relative bg-neutral-50/30">
        ...
        <div className="h-[90px] shrink-0 w-full bg-white/80 backdrop-blur-3xl border-t border-neutral-200/60 flex items-center justify-between px-4 md:px-8 z-50">
          ...
          <div className="flex flex-col items-center justify-center w-2/4 md:w-1/3 gap-1.5 md:gap-2">
            <div className="flex items-center gap-4 md:gap-6">
              ...
              <button onClick={togglePlay} className="w-10 h-10 md:w-11 md:h-11 bg-neutral-900 text-white rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-md shadow-neutral-900/10">
          ...
          <div className="flex items-center justify-end gap-3 md:gap-4 w-1/4 md:w-1/3">
            ...
            <div className="w-16 md:w-24 hidden sm:flex items-center">
              <input type="range" min="0" max="1" step="0.01" value={volume !== undefined ? volume : 0.8} onChange={(e) => setVolume && setVolume(parseFloat(e.target.value))} className="w-full h-1.5 bg-neutral-200 rounded-full appearance-none cursor-pointer accent-neutral-900" />
            </div>
          </div>
        </div>
      </div>
