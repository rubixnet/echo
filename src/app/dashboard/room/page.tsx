...
export default function LiveRoomsPage() {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8 min-h-screen pb-32">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-neutral-200/40 pb-6">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            <h2 className="text-2xl font-black text-neutral-900 tracking-tight">Live Rooms</h2>
            <Badge variant="secondary" className="bg-emerald-50 text-emerald-700 hover:bg-emerald-50 border-emerald-200/50 font-extrabold text-[10px] uppercase tracking-wider px-2 py-0.5">
              <Radio size={10} className="mr-1 animate-pulse" /> {liveRooms.length} Active
            </Badge>
          </div>
          <p className="text-sm text-neutral-500 font-medium">Jump into a perfectly synchronized audio stream with creators around the world.</p>
        </div>
      </div>
          ...
          {liveRooms.map((room) => (
            <Card key={room._id} className="bg-white border-neutral-200/60 shadow-sm hover:shadow-md transition-all rounded-2xl overflow-hidden group flex flex-col justify-between">
              ...
              <CardFooter className="bg-neutral-50/50 border-t border-neutral-100 p-4 flex items-center justify-between gap-4 shrink-0">
                ...
                {/* APPLIED buttonVariants TO THE LINK */}
                <Link 
