...
export default function SearchPage() {
  ...
  const myRoom = useQuery(api.rooms.getMyHosterRooms, { userId: user._id });
  ...
  const handlePlayYouTube = async (ytTrack: any) => {
    const videoId = ytTrack.url.split("?v=")[1] || ytTrack.url.split("/v/")[1] || ytTrack.url.split("youtu.be/")[1] || "YQHsXMglC9A";
    ...
    try {
      ...
      const durationStr = `${Math.floor(ytTrack.duration / 60)}:${(ytTrack.duration % 60).toString().padStart(2, "0")}`;

      const newTrackId = await ensureYoutubeTrack({
        youtubeId: videoId,
        title: ytTrack.title,
      ...
      if (myRoom) {
        await updateRoomTrack({ roomId: myRoom._id, trackId: newTrackId });
      }
      ...
      setActiveMetadata((prev: any) => prev ? { ...prev, id: newTrackId } : prev);
          ...
          <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="divide-y divide-neutral-100/50">
              {ytResults.map((track, index) => {
                ...
                return (
                  <div key={videoId} onClick={() => handlePlayYouTube(track)} className="flex items-center justify-between py-2.5 group cursor-pointer hover:bg-neutral-100/40 px-2 -mx-2 rounded-xl transition-all">
