...
export async function GET(request: Request) {
    ...
    if (!id) return NextResponse.json({ error: "No video ID provided" }, { status: 400 });
    ...
    const getYoutubeCaptions = async () => {
        ...
        let track = tracks.find((t: any) => t.vssId === ".en" || t.languageCode === "en" || t.vssId === "a.en");
    ...
    try {
        ...
        return NextResponse.json({ error: "No lyrics found anywhere" }, { status: 404 });
    }
}