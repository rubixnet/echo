...
import { LiquidPanel } from "@/components/LiquidGlassCard";
  ...
  return (
    <ContextMenuPrimitive.Portal>
      <ContextMenuPrimitive.Content
        ...
        className={cn(
          "z-50 min-w-48 overflow-hidden outline-none animate-in fade-in-0 zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
          className
        )}
        {...props}
      >
        <LiquidPanel radius="20px" className="w-full h-full shadow-2xl p-1.5">
          {children}
        </LiquidPanel>
      </ContextMenuPrimitive.Content>
    </ContextMenuPrimitive.Portal>
...
function ContextMenuSubContent({
  ...
  return (
    <ContextMenuPrimitive.SubContent
      ...
      className={cn("z-50 min-w-32 overflow-hidden outline-none animate-in fade-in-0 zoom-in-95", className)}
      {...props}
    >
      <LiquidPanel radius="24px" className="w-full h-full shadow-2xl p-1.5">
        {children}
      </LiquidPanel>
    </ContextMenuPrimitive.SubContent>
  );
...
function ContextMenuLabel({ className, inset, ...props }: React.ComponentProps<typeof ContextMenuPrimitive.Label> & { inset?: boolean }) {
  return <ContextMenuPrimitive.Label data-slot="context-menu-label" data-inset={inset} className={cn("px-3 py-2.5 text-xs text-muted-foreground data-inset:pl-9.5", className)} {...props} />;
}
