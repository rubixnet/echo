...
export function Track({
  ...
  const dislikeSong = useMutation(api.dislikes.toggleDislike);
  ...
  const suggestLess = useMutation(api.suggestLess.suggestLessTracks);
  ...
  const renderMenuItems = (Item: any, Separator: any) => (
    <div className="flex flex-col w-full text-primary">
      <div className={cn("flex items-center justify-around border-b border-primary/10", isMobile ? "px-6 py-4" : "px-4 py-3")}>
        <button
          onClick={(e: any) => {
            e.stopPropagation();
            handleLike();
          }}
          className={cn("hover:scale-110 active:scale-95 transition-all outline-none", isLiked ? "text-emerald-500" : "text-primary")}
          title={isLiked ? "Remove from Favorite" : "Add to Favorite"}
        >
          <Star size={isMobile ? 22 : 18} className={cn("transition-colors", isLiked ? "fill-emerald-500 text-emerald-500" : "text-primary/80")} />
        </button>
        <button
          onClick={(e: any) => {
            e.stopPropagation();
          ...
          className={cn("hover:scale-110 active:scale-95 transition-all outline-none", isBookmarked ? "text-primary" : "text-primary")}
          title={isBookmarked ? "Remove from Library" : "Save to Library"}
        >

          <Bookmark size={isMobile ? 22 : 18} className={cn("transition-colors", isBookmarked ? "fill-primary text-primary" : "text-primary/80")} />
        </button>
        <button
      ...
      <div className={cn("flex flex-col", isMobile ? "p-2 gap-0.5" : "p-1.5 gap-0.5")}>
        ...
        <Separator className={cn("bg-primary/10 mx-2", isMobile ? "my-1.5" : "my-1")} />
        ...
        <Separator className={cn("bg-primary/10 mx-2", isMobile ? "my-1.5" : "my-1")} />
        <Item className={itemClassName} onClick={(e: any) => { e.stopPropagation(); handleSuggestLess(); }}>
          <ThumbsDown size={iconSize} className="text-primary/70" /> Suggest Less
        </Item>
        <Item className={itemClassName}>
          <Ban size={iconSize} className="text-primary/70" /> Never Show Me This Again!
        </Item>
        {/* <Item className={itemClassName}>
            ...
            >
              <div className="relative aspect-square w-full rounded-2xl overflow-hidden bg-neutral-100 shadow-sm">
                <img
                  ...
                  onError={(e) => {
                    e.currentTarget.src = "https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17?q=80&w=256";
                  }}
                />
                <div className={cn("absolute inset-0 flex items-center justify-center transition-all duration-300", isCurrent ? "bg-black/40 opacity-100" : "bg-black/0 opacity-0 group-hover:bg-black/20 group-hover:opacity-100")}>
                  <div className={cn("w-12 h-12 flex items-center justify-center rounded-full bg-emerald-500 text-primary shadow-xl transform transition-transform duration-300", isCurrent || isLoading ? "scale-100" : "scale-75 translate-y-4 group-hover:scale-100 group-hover:translate-y-0")}>
                    {isLoading ? <Loader2 size={24} className="animate-spin" /> : isCurrent ? <Pause size={24} className="fill-primary" /> : <Play size={24} className="fill-primary ml-1" />}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3.5 min-w-0 flex-1">
                ...
                <div className="relative w-11 h-11 overflow-hidden shrink-0 border rounded-sm border-neutral-200 shadow-sm bg-black/50 p-0.5">
                  ...
                  <div className={cn("absolute inset-0 flex items-center justify-center transition-all duration-200 rounded-xl", isCurrent ? "bg-black/30 opacity-100" : "bg-neutral-950/20 opacity-0 group-hover:opacity-100")}>
                    {isLoading ? <Loader2 size={14} className="text-primary animate-spin" /> : isCurrent ? <Pause size={14} className="text-primary fill-primary" /> : <Play size={14} className="text-primary fill-primary ml-0.5" />}
                  </div>
                </div>
