...
import { Timeline, PlaybackControls, LikeButton, useNextInQueue, VibrantBackground, PlaybackStatus } from "./Shared";
import { Track } from "../TrackComponent";
import { ChevronDown, Maximize2, Minimize2, Music, EllipsisVertical, Loader2, Image as ImageIcon, Mic2, ListMusic } from "lucide-react";
...
export function DesktopDrawer({ isOpen, setIsOpen }: { isOpen: boolean, setIsOpen: (v: boolean) => void }) {
    ...
    useEffect(() => {
        ...
        const handleMouseMove = (e: MouseEvent) => {
            ...
            if (newWidth > 280 && newWidth < window.innerWidth - 100) setDrawerWidth(newWidth);
        };
    ...
    return (
        ...
        >
            ...
            {!isFullscreen && (
                <div onMouseDown={() => setIsResizing(true)} className="absolute left-0 top-0 w-1.5 h-full cursor-ew-resize hover:bg-foreground/10 active:bg-foreground/20 z-[1001] transition-colors" />
            )}

            <div className="flex items-center justify-between p-3 shrink-0 relative z-10">
                <div className="flex-1 flex justify-start">
                    <button onClick={() => setIsOpen(false)} className="p-1.5 text-foreground/50 hover:text-foreground hover:bg-foreground/5 rounded-full transition-colors">
                ...
                <div className="flex-1 flex justify-end">
                    <button onClick={() => setIsFullscreen(!isFullscreen)} className="p-1.5 text-foreground/50 hover:text-foreground hover:bg-foreground/5 rounded-full transition-colors">
                    ...
                    <>
                        {activeTab === 'lyrics' && (
                            <div className="flex-1 overflow-hidden flex flex-col max-w-xl mx-auto w-full relative">
                                <SyncedLyrics activeMetadata={activeMetadata} currentTimeSec={currentTimeSec} seekToTime={seekToTime} />
                            </div>
                        )}
                        {activeTab === 'cover' && (
            ...
            {activeMetadata && (
                <div className={cn("px-5 pb-5 pt-2 shrink-0 relative z-10 flex flex-col", isFullscreen ? "max-w-2xl w-full mx-auto" : "")}>
                    <div className="flex items-center justify-between mb-3">
                        <div className="flex flex-col min-w-0 pr-4">
                            <h2 className="text-sm font-bold text-foreground truncate tracking-tight mb-0.5">{activeMetadata.title}</h2>
                        ...
                        <div className="flex items-center gap-0.5 shrink-0 text-foreground/60">
                            <div className="scale-75 origin-center">
                                <LikeButton className="p-1" />
                            </div>
                            <button className="p-1.5 hover:text-foreground hover:bg-foreground/5 rounded-full transition-colors">
