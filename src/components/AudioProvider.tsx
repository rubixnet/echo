...
export function AudioProvider({ children }: { children: React.ReactNode }) {
  ...
  const [activeMetadata, setActiveMetadata] = useState<TrackMetadata | null>(
    null,
  );
      ...
      if (p !== undefined) {
        p.catch((err) => {
          if (err.name !== "AbortError") {
            console.warn("[AudioEngine] Playback promise rejected:", err);
          }
        });
      }
    } else {
  ...
  const loadTrack = (url: string, metadata?: TrackMetadata | any) => {
    ...
    if (!normalized.id) {
      console.error(
        "[AudioEngine] Refusing playback: No valid YouTube ID found",
        metadata,
      );
      setIsLoading(false);
      setIsPlaying(false);
  ...
  const getCurrentTime = () => {
    ...
    if (!isYouTube && nativeAudioRef.current)
      return nativeAudioRef.current.currentTime || 0;
    return 0;
  };
    ...
    >
      <audio
        ...
        onError={(e) => {
          if (isYouTube) return;
          console.error(
            "[AudioEngine] Native audio stream error (403/500/Format unplayable):",
            e,
          );
          setIsLoading(false);
          setIsAudioReady(false);
          onTrackErrorRef.current(e);
        }}
        onEnded={() => {
          if (!isYouTube) {
            setIsPlaying(false);
